# IGUAL QUE html_to_csv_WEB pero añade las ciudades y comunidades autonomas
# al resultado
...
def extract_data_from_html(html_content):
    ...
    for result in results:
        ...
        # Extracción de la ciudad desde la etiqueta <meta>
        city_tag = result.find('meta', itemprop='addressLocality')
        if city_tag:
            city = city_tag.get('content', 'No especificado').strip()
        else:
            city = "No especificado"
        psychologist_info['Ciudad'] = city
        
        # Extracción de la comunidad autónoma/provincia desde la etiqueta <meta>
        province_tag = result.find('meta', itemprop='addressRegion')
        if province_tag:
            province = province_tag.get('content', 'No especificado').strip()
        else:
            province = "No especificado"
        psychologist_info['Comunidad/Provincia'] = province
...
def write_data_to_csv(data, csv_file_path):
    with open(csv_file_path, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=['Identificación', 'Especialidades', 'Subespecialidades', 'URL del perfil', 'Precio', 'Ciudad', 'Comunidad/Provincia'])
        writer.writeheader()
        for row in data:
            writer.writerow(row)
